...
public class CalculateRent extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        return inflater.inflate(R.layout.activity_showrentcalculate, container, false);
    }
    ...
    public void onViewCreated(View view, @Nullable Bundle savedInstanceState) {
        ...
        getActivity().setTitle("Rent Information");
    ...
    void showExpenseList() {
        try {
            ...
            postRequest(postUrl + "/calculaterent", userInfo.toString());
        } catch (Exception ex) {
                ...
                try {
                    ...
                    if (s != null) {
                        if (resPonse.length() > 0) {
                            ...
                            TableLayout tableLayout = (TableLayout) getActivity().findViewById(R.id.rentTableLayout);

                            if  (jsonArray.length() == 0 )
                            {
                                Toast.makeText(getContext(), "No expenses added from last successful rent calculation.", Toast.LENGTH_SHORT).show();
                                return;
                            }
                            ...
                            for (int nIndex = 0; nIndex < jsonArray.length(); nIndex++) {
                                ...
                                String[] rowData = {
                                        jsonRowData.get("userName").toString(),
                                        jsonRowData.get("averagePerHead").toString(),
                                        jsonRowData.get("commonPayable").toString(),
                                        jsonRowData.get("amountspent").toString(),
                                        jsonRowData.get("amountPerHead").toString()
                                };
                            ...
                            Toast.makeText(getContext(), "Failed to calculate rent.", Toast.LENGTH_SHORT).show();
                        }
                    }
